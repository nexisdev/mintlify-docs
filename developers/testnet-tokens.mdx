...
| **WebSocket** | `wss://ws.testnet.nexis.network` |
| **Block Explorer** | `https://explorer.testnet.nexis.network` |
| **Bridge** | `https://bridge.testnet.nexis.network` |
...
// Just connect to https://app.nexis.network and approve the network switch
```
...
<Steps>
  <Step title="Visit Bridge">
    Go to [https://bridge.testnet.nexis.network](https://bridge.testnet.nexis.network)
  </Step>
  <Step title="Connect Wallet">
...
async function checkBridgeStatus(l1TxHash) {
  const response = await fetch(
    `https://bridge.testnet.nexis.network/api/status/${l1TxHash}`
  );
...
**URL:** [https://faucet.nexis.network](https://faucet.nexis.network)
...
async function claimFromFaucet(address) {
  const response = await fetch('https://faucet.nexis.network/api/claim', {
    method: 'POST',
    headers: {
...
def claim_faucet(address: str) -> dict:
    ...
    url = 'https://faucet.nexis.network/api/claim'
...
FAUCET_URL="https://faucet.nexis.network/api/claim"
...
const faucet = new FaucetManager({
  url: 'https://faucet.nexis.network',
...
```bash
# Using curl
curl -X POST https://rpc.testnet.nexis.network \
  -H "Content-Type: application/json" \
  -d '{
    "jsonrpc": "2.0",
...
w3 = Web3(Web3.HTTPProvider('https://rpc.testnet.nexis.network'))
...
4. Check RPC endpoint is reachable: `curl https://rpc.testnet.nexis.network`
...
async function canAffordTransaction(address, contractAddress, calldata) {
  ...
  if (balance < cost) {
    ...
    console.log(`Visit: https://faucet.nexis.network`);
    return false;
  }
...
<CardGroup cols={2}>
  <Card title="Official Faucet" icon="faucet" href="https://faucet.nexis.network">
    Get NZT testnet tokens
  </Card>
  <Card title="Bridge" icon="bridge" href="https://bridge.testnet.nexis.network">
    Bridge from Base Sepolia
  </Card>
  <Card title="Block Explorer" icon="magnifying-glass" href="https://explorer.testnet.nexis.network">
